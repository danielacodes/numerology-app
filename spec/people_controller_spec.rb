...
describe "Our Person Controller" do
  ...
  before (:all) do
    ...
    @last_name = "Piggy"
    @birthday = '1972-05-01'
    @person = Person.create(first_name: @first_name, last_name: @last_name, birthday: @birthday)
  end
  ...
  it 'creates a person when we post to /people' do
    ...
    post("/people/", { first_name: "Kermit", last_name: "TheFrog", birthday: (DateTime.now - 45.years) })
    expect(Person.all.count == people_count + 1).to eq(true)
  end

  it 'edits a person when we put to /people/:id' do
    new_first_name = "Mister"
    put("/people/#{@person.id}", { first_name: new_first_name, last_name: @last_name, birthday: @birthday })
